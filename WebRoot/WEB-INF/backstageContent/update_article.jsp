...
<head>
	...
	<script>
		$.get(
			...
			function(data) {
				...
				for(var i = 0; i < list.length; i ++) {
					var option;
					if(list[i].typeId == ${article.article_type_id}){
						option = $("<option value="+list[i].typeId+" selected>"+list[i].typeName+"</option> ");
					}else{
						option = $("<option value="+list[i].typeId+">"+list[i].typeName+"</option> ");
					}
					console.log(option);
		...
		function avoidRepeat(){
			...
			$.post(
				"${pageContext.request.contextPath}/articleServlet?method=updateAvoidRepeat",
				{title:title,id:'${article.id }'},
				function(data){
					$("#errorMes").html(data);
				}
			);
...
<body>
	<div>
          ...
          <div class="table-responsive">
            <form id="articleForm" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath }/articleServlet?method=update">
            	...
            	<div>
            	<label for="">标题</label>
            	<input id="title" onblur="avoidRepeat()" name="title" type="text" class="form-control" value="${article.title }">  
            	<span id="titleMes" style="color: red;"></span>
				<span id="errorMes" style="color: red;"></span>
            	</div>
            	<div>
            	<label  for="">关键字</label>
            	<input id="keyword" name="keyword" type="text" class="form-control" value="${article.keyword }">  
            	<span id="keywordMes" style="color: red;"></span>
            	</div>
            	<br><br>
				<input name="clickRate" type="hidden" value="${article.clickRate }">
               <textarea name="content" id="mytextarea">${article.content }</textarea>
               <!-- <button type="submit" class="btn btn-primary">&nbsp;&nbsp;&nbsp;&nbsp;提交&nbsp;&nbsp;&nbsp;&nbsp;</button> -->
               <a class="btn btn-primary" href="javascript:void(0);" onclick="articleUpdate()">&nbsp;&nbsp;&nbsp;&nbsp;更新&nbsp;&nbsp;&nbsp;&nbsp;</a>
            </form>
          </div>
        </div>
</body>
